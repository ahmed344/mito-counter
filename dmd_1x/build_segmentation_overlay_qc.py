...
DEFAULT_INPUT_ROOT = REPO_ROOT / "data" / "DMD_1X" / "Processed"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "data" / "DMD_1X" / "results" / "overlay_qc"
MITO_ALPHA = 0.28
...
class OverlayInput:
    """Container for one DMD_1X overlay rendering job.
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the DMD_1X QC overlay script.
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Render DMD_1X full-frame QC overlays by alpha-blending mitochondria "
...
def strip_source_suffix(path: Path) -> str:
    """Return the image label for either plain or DMD-style corrected source names.
...
def build_overlay_input(
    ...
    """Resolve sibling files and output path for one DMD_1X source image.
...
def render_overlay(source_image: np.ndarray, segmented_rgb: np.ndarray) -> np.ndarray:
    """Render a DMD_1X QC overlay from a source image and segmentation visualization.
...
def process_one_overlay(job: OverlayInput) -> None:
    """Render and save one DMD_1X overlay image.
...
def main() -> None:
    """Run the DMD_1X batch QC overlay rendering pipeline.
