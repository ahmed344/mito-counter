"""Generate DMD_1X Bayesian diagnostics, PPCs, and image-level superplots."""
...
if str(DMD_DIR) not in sys.path:
    sys.path.insert(1, str(DMD_DIR))
...
c3_vis = load_module_from_path("c3_hierarchical_bayes_visualization_for_dmd_1x", C3_DIR / "hierarchical_bayes_visualization.py")
dmd_1x_metrics = load_module_from_path(
    "dmd_1x_hierarchical_bayes_metrics_for_visuals",
    DMD_1X_DIR / "hierarchical_bayes_metrics.py",
)
dmd_1x_config = dmd_1x_metrics.dmd_1x_config

BAYES_SCRIPT_PATH = DMD_1X_DIR / "hierarchical_bayes_metrics.py"
...
def short_compartment_label(compartment: str) -> str:
    ...
    if str(compartment) == dmd_1x_metrics.SS_LABEL:
        return "SS"
    if str(compartment) == dmd_1x_metrics.IMF_LABEL:
        return "IMF"
...
def dmd_1x_fit_title(row: pd.Series) -> str:
    """Build a DMD_1X figure title for one fit.
...
def dmd_1x_posterior_plot_specs(row: pd.Series) -> list[tuple[str, str, str, str]]:
    """Describe biology-facing posterior plots for DMD_1X fits.
    ...
    specs = [
        ...
        (
            ...
            "DMD-WT delta in image variance",
        ),
    ]
...
def figure_filename_prefix(row: pd.Series) -> str:
    """Build a flat filename prefix for one DMD_1X fit.
...
c3_vis.fit_title = dmd_1x_fit_title
c3_vis.posterior_plot_specs = dmd_1x_posterior_plot_specs
c3_vis.figure_filename_prefix = figure_filename_prefix


def parse_args() -> argparse.Namespace:
    """Parse command-line options for DMD_1X Bayesian visualization.
    ...
    parser = argparse.ArgumentParser(
        description="Generate DMD_1X Bayesian diagnostics and image-level superplots."
    )
...
def filter_summary_to_fit_configs(summary_df: pd.DataFrame, fit_configs: list[Any]) -> pd.DataFrame:
    """Filter summary rows to configured repeated DMD_1X fits.
...
def resolve_trace_path(row: pd.Series, trace_dir: Path) -> Path:
    """Resolve the trace path for one DMD_1X summary row.
    ...
    return dmd_1x_metrics.trace_path_for_fit(
        trace_dir=trace_dir,
        muscle=str(row["muscle"]),
        compartment=str(row["compartment"]),
        metric=str(row["metric"]),
...
def rerun_bayesian_fits(config_path: Path) -> None:
    """Invoke the DMD_1X fitting script for missing visualization inputs.
...
def load_or_refresh_summary(
    ...
    """Load the summary CSV, optionally rerunning DMD_1X fits first.
    ...
    if refresh_mode == "rerun_missing_traces" and traces_missing(summary_df, trace_dir):
        rerun_bayesian_fits(config_path=config_path)
        summary_df = filter_summary_to_fit_configs(
            summary_df=load_summary(summary_path),
            fit_configs=fit_configs,
...
def resolved_likelihood_name(row: pd.Series, fit_config: Any) -> str:
    """Return the likelihood name used by one saved DMD_1X fit.
...
def prepare_data_for_row(
    ...
    """Rebuild prepared DMD_1X arrays for one visualization row.
    ...
    positive_likelihood = likelihood_name if family == "positive" else dmd_1x_metrics.POSITIVE_LIKELIHOODS[0]
    bounded_likelihood = likelihood_name if family == "bounded" else dmd_1x_metrics.BOUNDED_LIKELIHOODS[1]
    real_likelihood = likelihood_name if family == "real" else dmd_1x_metrics.REAL_LIKELIHOODS[0]
...
def bayesian_superplot_annotations(
    ...
    """Build DMD_1X superplot annotation records for one comparison.
...
def plot_bayesian_superplots(
    ...
    """Generate DMD_1X image-level Bayesian superplots for one fit.
    ...
    if subset.empty:
        print(f"Skipping superplots for {dmd_1x_fit_title(row)}: no matching rows.")
        return
...
def generate_fit_visualizations(
    ...
    """Generate all requested DMD_1X visualizations for one fit.
...
def run_visualization_analysis(config_path: Path, config: Any) -> None:
    """Generate figures for one configured DMD_1X Bayesian analysis.
    ...
    if not fit_configs:
        ...
        return
    measurements_df = dmd_1x_metrics.load_measurements(config.paths, config.filters)
...
def main() -> None:
    """Run the DMD_1X Bayesian visualization workflow.
    ...
    args = parse_args()
    config = dmd_1x_metrics.load_config(args.config)
